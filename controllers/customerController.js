...
export const addCustomer = async (req, res) => {
    ...
    try {
        const checkQuery =
            "SELECT * FROM customers WHERE name = ? AND phone = ? AND admin_id = ?";
            ...
            db.getConnection((connErr, connection) => {
                if (connErr) {
                    return res.status(500).json({
                        success: false,
                        message: "Database connection failed for transaction",
                    });
                }
                ...
                connection.beginTransaction((transactionErr) => {
                    if (transactionErr) {
                        ...
                        return res
                            .status(500)
                            .json({ success: false, message: "Transaction failed to start" });
                    }
                                    ...
                                    if (vehErr) {
                                        return connection.rollback(() => {
                                            ...
                                            res.status(500).json({
                                                success: false,
                                                message:
                                                    "Customer registered, but error saving vehicle data",
                                            });
                                        });
                                    }
                ...
                (custErr, custResult) => {
                    ...
                    if (brand && model && vehicle_number) {
                                ...
                                (vehErr, vehResult) => {
                                    if (vehErr) {
                                        return connection.rollback(() => {
                                            ...
                                            res.status(500).json({
                                                success: false,
                                                message: "Error updating existing vehicle data",
                                            });
                                        });
                                    }
                                    ...
                                    commitTransaction(
                                        connection,
                                        res,
                                        "Customer and Vehicle updated successfully",
                                    );
                                },
                            );
                        } else {
                                ...
                                (checkErr, checkResults) => {
                                    ...
                                    if (checkResults.length > 0) {
                                        return commitTransaction(
                                            connection,
                                            res,
                                            "Customer updated successfully (Vehicle already exists, no changes made)",
                                        );
                                    }
                                    ...
                                    connection.query(
                                        ...
                                        (insertVehErr, insertVehResult) => {
                                            if (insertVehErr) {
                                                return connection.rollback(() => {
                                                    ...
                                                    res.status(500).json({
                                                        success: false,
                                                        message:
                                                            "Customer updated, but error adding new vehicle",
                                                    });
                                                });
                                            }
                                            ...
                                            commitTransaction(
                                                connection,
                                                res,
                                                "Customer updated and new vehicle added successfully",
                                            );
                                        },
                                    );
                                },
                        ...
                        commitTransaction(
                            connection,
                            res,
                            "Customer updated successfully (No vehicle changes)",
                        );
                    }
                },
            );
    ...
    connection.commit((commitErr) => {
        if (commitErr) {
            return connection.rollback(() => {
                ...
                res
                    .status(500)
                    .json({ success: false, message: "Transaction commit failed" });
            });
        }
        connection.release();
